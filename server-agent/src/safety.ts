...
const SYSTEM_ROOTS = ["/", "/etc", "/root", "/home", "/usr", "/var", "/opt", "/proc", "/sys", "/dev", "/run", "/tmp"];

export function normalizeWorkspaceRoot(root: string): string {
  const resolved = path.resolve(root || ".");
  if (SYSTEM_ROOTS.includes(resolved)) {
    throw Object.assign(new Error(`Refusing unsafe workspace root: ${resolved}`), { statusCode: 400 });
  }
  return resolved;
}
...
export function assertSafeRelativePath(inputPath = ".", workspaceRoot: string): string {
  ...
  if (SECRET_PATTERNS.some((pattern) => pattern.test(rel))) {
    throw Object.assign(new Error("Blocked access to a secret or system path."), { statusCode: 403 });
  }
  return resolved;
}
...
export function assertSafeCommand(command: string): void {
  ...
  if (/\b(printenv|env)\b|\.env|\.ssh|\.aws|id_rsa|id_ed25519|credentials|secret/i.test(command)) {
    throw Object.assign(new Error("Command appears to access secrets and was blocked."), { statusCode: 403 });
  }
}
