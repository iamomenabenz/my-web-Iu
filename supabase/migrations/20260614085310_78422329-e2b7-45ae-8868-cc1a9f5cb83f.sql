...
-- Server-side writes only (review fix: restrict tool_executions inserts to service role).
GRANT SELECT ON public.tool_executions TO authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.tool_executions FROM authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.tool_executions FROM anon;
GRANT ALL ON public.tool_executions TO service_role;
ALTER TABLE public.tool_executions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "tool executions read own or admin" ON public.tool_executions;
